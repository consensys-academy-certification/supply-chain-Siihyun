...
contract SupplyChain {
  ...
  modifier checkState (uint _id, State _state){
    require(items[_id].state == _state, "state check fail");
    _;
  }
  ...
  modifier checkCaller (uint _id, bool isSeller){
    if(isSeller) require(items[_id].seller == msg.sender, "you are not seller");
    else require(items[_id].buyer == msg.sender, "you are not buyer");
    _;
  }
  ...
  function addItem (string memory _name, uint _price) checkValue(PAY_FEE) checkParam(_name, _price) public payable{
    ...
    emit log(itemIdCount-1, State.ForSale);
  }
